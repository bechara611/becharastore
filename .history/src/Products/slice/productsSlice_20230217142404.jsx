...
export const productSlice = createSlice({
  ...
  initialState,
  reducers: {
    //?Este metodo del slice activa las categorias
    onAsignarCategoriasAll:(state,action)=>{
    ...
    onAsignarProductosAll: (state, { payload }) => {
      ...
      for (let i = 0; i < vueltas; i++) {
        if(state.productosAll[i]?.cantidad>0){
    ...
    onAsignar5productos: (state, { payload }) => {
      ...
      for (let i = payload.inicio; i < payload.final; i++) {
        let existe = state.productoscargando5.filter(producto=>{ return producto?._id===state.productosAll[i]?._id})
        if (state?.productosAll[i]?.urlimg && !existe ) {
          state.productoscargando5.push(state.productosAll[i])
        }else{
          state.productoscargando5.push(undefined)
        }
...
export const { onAsignarProductosAll, onAsignar5productos, onReset5Products, onFiltrarProductos,onAsignarCategoriasAll } = productSlice.actions;