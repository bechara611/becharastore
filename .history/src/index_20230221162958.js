...
const App2=()=>{
    ...
    const validaciones = {
        Nombre:[(value)=>value.length>0,'INSERT A VALID NOMBRE'],
        Email:[(value)=>value.includes('@'),'INSERT A VALID EMAIL']
    }
    const {Nombre,Email,onChangeValor,NombreValid,EmailValid,isFormValid} = useFormularioDany(initialForm,validaciones)
    return(
        <>
        <h1>App2</h1>
        <form action="">
        ...
        <button onClick={()=>{console.log({Nombre,Email,NombreValid,EmailValid,isFormValid})}}>dsfsdf</button>
        </>
    )
}
